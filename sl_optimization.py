"""
sl_optimization.py
------------------
Confronta Bloomberg V2 con diversi moltiplicatori SL e R:R per trovare
la configurazione ottimale che massimizza EV per trade e win rate.

Baseline: SL = 1.5×ATR, TP = 4×SL  (R:R 1:4)

Domanda: vale la pena allargare lo SL e ridurre il R:R?

Uso:
    python sl_optimization.py
"""

import os
import pickle
import pandas as pd
from datetime import datetime
from analyzer import compute_indicators, score_stock
from config import WATCHLIST_USA, WATCHLIST_EUROPE
import yfinance as yf

# ---------------------------------------------------------------------------
# Parametri globali
# ---------------------------------------------------------------------------

CAPITAL    = 20_000
TRADE_SIZE = 500
SAFETY_CAP = 52
MIN_BARS   = 26
PRE_FILTER_N = 20
TOP_N = 3

BT_START = "2020-01-01"
BT_END   = "2026-04-30"

# Trade Republic: 1 EUR/ordine × 2 = 2 EUR round-trip, no swap
COMMISSION_EUR = 2.0
SWAP_WEEKLY    = 0.0

CACHE_FILE = os.path.join(os.path.dirname(__file__), "market_data.pkl")

# ---------------------------------------------------------------------------
# Configurazioni da testare: (sl_mult, rr, label)
# ---------------------------------------------------------------------------

CONFIGS = [
    # SL stretto (baseline)
    (1.5, 4.0, "SL=1.5×ATR  R:R 1:4  [baseline]"),
    (1.5, 3.0, "SL=1.5×ATR  R:R 1:3"),
    (1.5, 2.0, "SL=1.5×ATR  R:R 1:2"),
    # SL medio
    (2.0, 4.0, "SL=2.0×ATR  R:R 1:4"),
    (2.0, 3.0, "SL=2.0×ATR  R:R 1:3"),
    (2.0, 2.0, "SL=2.0×ATR  R:R 1:2"),
    # SL largo
    (2.5, 4.0, "SL=2.5×ATR  R:R 1:4"),
    (2.5, 3.0, "SL=2.5×ATR  R:R 1:3"),
    (2.5, 2.0, "SL=2.5×ATR  R:R 1:2"),
    # SL molto largo
    (3.0, 4.0, "SL=3.0×ATR  R:R 1:4"),
    (3.0, 3.0, "SL=3.0×ATR  R:R 1:3"),
    (3.0, 2.0, "SL=3.0×ATR  R:R 1:2"),
]


# ---------------------------------------------------------------------------
# Bloomberg V2 score
# ---------------------------------------------------------------------------

def bloomberg_enhanced_score(ind: dict) -> float:
    base   = score_stock(ind)
    rsi    = ind.get("rsi_14") or 0
    vol    = ind.get("volume_ratio") or 0
    change = ind.get("weekly_change_pct") or 0
    sma50  = ind.get("above_sma50") or False

    bonus = 0.0
    if vol >= 1.5:          bonus += 2.0
    elif vol >= 1.2:        bonus += 0.5
    if 50 <= rsi <= 62:     bonus += 1.5
    elif 48 <= rsi < 50:    bonus += 0.5
    elif rsi > 65:          bonus -= 2.0
    if change >= 2.0:       bonus += 1.5
    elif change >= 1.0:     bonus += 0.5
    if sma50:               bonus += 1.0

    return base + bonus


# ---------------------------------------------------------------------------
# Cache dati
# ---------------------------------------------------------------------------

def load_cache(tickers):
    if os.path.exists(CACHE_FILE):
        age_days = (datetime.now().timestamp() - os.path.getmtime(CACHE_FILE)) / 86400
        if age_days < 7:
            print(f"Caricamento dati da cache ({age_days:.1f} giorni fa)...")
            with open(CACHE_FILE, "rb") as f:
                data = pickle.load(f)
            print(f"Asset in cache: {len(data)}/{len(tickers)}\n")
            return data
        print(f"Cache scaduta ({age_days:.1f} giorni), re-download...")

    print(f"Download dati WEEKLY {BT_START} -> {BT_END}")
    data = {}
    total = len(tickers)
    for i, ticker in enumerate(tickers, 1):
        print(f"\r  {i}/{total}: {ticker:<14}", end="", flush=True)
        try:
            df = yf.download(
                ticker, start="1999-01-01", end=datetime.today().strftime("%Y-%m-%d"),
                interval="1wk", progress=False, auto_adjust=True
            )
            if not df.empty:
                df.columns = [c[0] if isinstance(c, tuple) else c for c in df.columns]
                data[ticker] = df
        except Exception:
            pass
    print()
    with open(CACHE_FILE, "wb") as f:
        pickle.dump(data, f)
    print(f"Asset scaricati: {len(data)}/{len(tickers)}\n")
    return data


# ---------------------------------------------------------------------------
# Simulazione singolo trade
# ---------------------------------------------------------------------------

def simulate_trade(future, entry, sl, tp):
    for i, (_, row) in enumerate(future.iterrows()):
        if i >= SAFETY_CAP:
            return "TIMEOUT", float(future["Close"].iloc[i - 1]), i
        if float(row["Low"]) <= sl:
            return "LOSS", sl, i + 1
        if float(row["High"]) >= tp:
            return "WIN", tp, i + 1
    n = len(future)
    if n:
        return "OPEN", float(future["Close"].iloc[-1]), n
    return "OPEN", entry, 0


# ---------------------------------------------------------------------------
# Backtest Bloomberg V2 con parametri configurabili
# ---------------------------------------------------------------------------

def run_backtest(all_data, sl_mult, rr):
    signal_dates = pd.date_range(BT_START, BT_END, freq="W-FRI")
    trades = []

    for sig_date in signal_dates:
        candidates = []

        for ticker, df in all_data.items():
            hist = df[df.index <= sig_date].tail(200)
            if len(hist) < MIN_BARS:
                continue
            try:
                ind   = compute_indicators(hist)
                score = score_stock(ind)
                if not (score > 0 and (ind.get("macd_hist") or 0) > 0):
                    continue
                candidates.append((ticker, ind, score))
            except Exception:
                continue

        candidates.sort(key=lambda x: x[2], reverse=True)
        top20 = candidates[:PRE_FILTER_N]
        final = sorted(top20, key=lambda x: bloomberg_enhanced_score(x[1]), reverse=True)

        for ticker, ind, _ in final[:TOP_N]:
            entry = ind["price"]
            atr   = ind.get("atr_14") or (entry * 0.02)
            sl    = round(entry - sl_mult * atr, 4)
            tp    = round(entry + rr * sl_mult * atr, 4)

            future = all_data[ticker][all_data[ticker].index > sig_date]
            if future.empty:
                continue

            outcome, exit_price, bars_held = simulate_trade(future, entry, sl, tp)
            pnl_pct = (exit_price - entry) / entry * 100

            commission_pct = COMMISSION_EUR / TRADE_SIZE * 100 if outcome != "OPEN" else 0.0

            trades.append({
                "date":          sig_date.strftime("%Y-%m-%d"),
                "year":          sig_date.year,
                "ticker":        ticker,
                "entry":         round(entry, 4),
                "sl_pct":        round((entry - sl) / entry * 100, 2),
                "tp_pct":        round((tp - entry) / entry * 100, 2),
                "outcome":       outcome,
                "bars_held":     bars_held,
                "pnl_pct_gross": round(pnl_pct, 4),
                "pnl_pct":       round(pnl_pct - commission_pct, 4),
            })

    return pd.DataFrame(trades)


# ---------------------------------------------------------------------------
# Statistiche
# ---------------------------------------------------------------------------

def compute_stats(df, sl_mult, rr):
    closed = df[df["outcome"].isin(["WIN", "LOSS", "TIMEOUT"])].copy()
    if closed.empty:
        return None

    wins    = closed[closed["outcome"] == "WIN"]
    losses  = closed[closed["outcome"] == "LOSS"]
    timeouts = closed[closed["outcome"] == "TIMEOUT"]

    wr = len(wins) / len(closed) * 100

    # EV teorico atteso
    avg_win_pct  = wins["pnl_pct"].mean() if not wins.empty else 0
    avg_loss_pct = losses["pnl_pct"].mean() if not losses.empty else 0
    ev_pct = (wr / 100) * avg_win_pct + (1 - wr / 100) * avg_loss_pct

    # Profitto totale Trade Republic (net, con 26% tasse)
    total_pnl_eur = (closed["pnl_pct"] / 100 * TRADE_SIZE).sum()
    tax = max(0, total_pnl_eur * 0.26)
    net_profit = total_pnl_eur - tax
    ann_profit = net_profit / 6.33

    # Max drawdown
    balance = CAPITAL
    min_balance = CAPITAL
    for _, row in closed.sort_values("date").iterrows():
        balance += row["pnl_pct"] / 100 * TRADE_SIZE
        min_balance = min(min_balance, balance)
    max_dd = (CAPITAL - min_balance) / CAPITAL * 100

    # Max SL consecutivi
    max_cl = curr = 0
    for _, row in closed.sort_values("date").iterrows():
        if row["outcome"] == "LOSS":
            curr += 1
            max_cl = max(max_cl, curr)
        else:
            curr = 0

    return {
        "sl_mult":       sl_mult,
        "rr":            rr,
        "trades":        len(closed),
        "wins":          len(wins),
        "losses":        len(losses),
        "timeouts":      len(timeouts),
        "win_rate":      round(wr, 1),
        "avg_win_pct":   round(avg_win_pct, 2),
        "avg_loss_pct":  round(avg_loss_pct, 2),
        "ev_pct":        round(ev_pct, 3),
        "ev_eur":        round(ev_pct / 100 * TRADE_SIZE, 2),
        "total_pnl_eur": round(total_pnl_eur, 0),
        "net_after_tax": round(net_profit, 0),
        "ann_net_eur":   round(ann_profit, 0),
        "ann_pct":       round(ann_profit / CAPITAL * 100, 1),
        "max_dd_pct":    round(max_dd, 1),
        "max_consec_l":  max_cl,
        "avg_bars":      round(closed["bars_held"].mean(), 1),
    }


# ---------------------------------------------------------------------------
# Report
# ---------------------------------------------------------------------------

def print_report(all_stats):
    SEP  = "=" * 110
    SEP2 = "-" * 106

    print(f"\n{SEP}")
    print(f"  OTTIMIZZAZIONE SL  |  Bloomberg V2  |  Trade Republic (2 EUR RT, no swap, 26% tasse)")
    print(f"  Capitale {CAPITAL:,} EUR  |  Posizione {TRADE_SIZE} EUR  |  Top {TOP_N}/sett.  |  2020-2026 (6.33 anni)")
    print(SEP)

    hdr = (f"  {'Config':<28} {'Trade':>6} {'WR%':>6} {'AvgWIN':>7} {'AvgLSS':>7} "
           f"{'EV%':>7} {'EV EUR':>7} {'AnnNet':>8} {'Ann%':>6} {'MaxDD':>7} {'MaxSL-':>7} {'AvgWks':>7}")
    print(hdr)
    print(f"  {SEP2}")

    baseline = None
    for s in all_stats:
        if s["sl_mult"] == 1.5 and s["rr"] == 4.0:
            baseline = s
            break

    for s in all_stats:
        label = f"SL={s['sl_mult']}xATR  R:R 1:{s['rr']:.0f}"
        is_bl = (s["sl_mult"] == 1.5 and s["rr"] == 4.0)

        wr_flag  = "  " if baseline is None else (" *" if s["win_rate"]  > baseline["win_rate"]  else "  ")
        ev_flag  = "  " if baseline is None else (" *" if s["ev_eur"]    > baseline["ev_eur"]    else "  ")
        ann_flag = "  " if baseline is None else (" *" if s["ann_net_eur"] > baseline["ann_net_eur"] else "  ")

        marker = "[BASE]" if is_bl else "      "

        print(
            f"  {label:<28} "
            f"{s['trades']:>6} "
            f"{s['win_rate']:>5.1f}%{wr_flag}"
            f"{s['avg_win_pct']:>+6.1f}%"
            f"{s['avg_loss_pct']:>+7.1f}%"
            f"{s['ev_pct']:>+7.3f}%"
            f"{s['ev_eur']:>+7.2f}"
            f"{s['ann_net_eur']:>+8.0f}E"
            f"{s['ann_pct']:>+6.1f}%"
            f"{s['max_dd_pct']:>+7.1f}%"
            f"{s['max_consec_l']:>7}"
            f"{s['avg_bars']:>7.1f}"
            f"  {marker}"
        )
        if s["rr"] == 2.0:
            print(f"  {SEP2}")

    print(SEP)
    print("  * = migliore del baseline (SL=1.5×ATR, R:R 1:4)")
    print(f"\n  Legenda: EV% = expected value per trade | AnnNet = profitto annuo netto tasse 26%")
    print(f"           MaxDD = max drawdown | MaxSL- = max SL consecutivi | AvgWks = durata media in settimane")
    print(SEP)

    # Ranking per EV EUR
    print(f"\n  TOP 5 per EV per trade (netto commiss. TR, lordo tasse):")
    ranked = sorted(all_stats, key=lambda x: x["ev_eur"], reverse=True)
    for i, s in enumerate(ranked[:5], 1):
        label = f"SL={s['sl_mult']}xATR R:R 1:{s['rr']:.0f}"
        print(f"  {i}. {label:<24} EV={s['ev_eur']:+.2f} EUR/trade  "
              f"WR={s['win_rate']:.1f}%  AnnNet={s['ann_net_eur']:+.0f} EUR/anno  "
              f"MaxDD={s['max_dd_pct']:.1f}%")

    print(f"\n  TOP 5 per profitto annuo netto:")
    ranked2 = sorted(all_stats, key=lambda x: x["ann_net_eur"], reverse=True)
    for i, s in enumerate(ranked2[:5], 1):
        label = f"SL={s['sl_mult']}xATR R:R 1:{s['rr']:.0f}"
        print(f"  {i}. {label:<24} AnnNet={s['ann_net_eur']:+.0f} EUR/anno  "
              f"WR={s['win_rate']:.1f}%  EV={s['ev_eur']:+.2f} EUR/trade  "
              f"MaxDD={s['max_dd_pct']:.1f}%")
    print(SEP)


# ---------------------------------------------------------------------------
# Entry point
# ---------------------------------------------------------------------------

if __name__ == "__main__":
    all_tickers = WATCHLIST_USA + WATCHLIST_EUROPE

    print("=" * 60)
    print(f"  sl_optimization.py  |  Bloomberg V2  |  {len(CONFIGS)} configurazioni")
    print("=" * 60)
    print()

    all_data = load_cache(all_tickers)

    all_stats = []
    total_cfgs = len(CONFIGS)

    for i, (sl_mult, rr, label) in enumerate(CONFIGS, 1):
        print(f"  [{i:2d}/{total_cfgs}] Backtest {label}...", flush=True)
        df = run_backtest(all_data, sl_mult=sl_mult, rr=rr)
        stats = compute_stats(df, sl_mult, rr)
        if stats:
            all_stats.append(stats)
            print(f"         Trade={stats['trades']}  WR={stats['win_rate']:.1f}%  "
                  f"EV={stats['ev_eur']:+.2f}EUR  AnnNet={stats['ann_net_eur']:+.0f}EUR")

            fname = f"backtest_sl{str(sl_mult).replace('.','')}_rr{str(int(rr))}.csv"
            df.to_csv(os.path.join(os.path.dirname(__file__), fname), index=False)

    print()
    print_report(all_stats)

    # Salva summary CSV
    summary = pd.DataFrame(all_stats)
    out_path = os.path.join(os.path.dirname(__file__), "sl_optimization_results.csv")
    summary.to_csv(out_path, index=False)
    print(f"\nRisultati salvati in: sl_optimization_results.csv")
